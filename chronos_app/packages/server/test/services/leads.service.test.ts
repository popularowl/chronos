...
export function leadsServiceTest() {
    describe('Leads Service', () => {
        ...
        describe('getAllLeads', () => {
            it('should return all leads for an agentflow', async () => {
                const mockLeads = [
                    { id: 'lead-1', agentflowid: 'flow-1', name: 'John Doe', email: 'john@example.com' },
                    { id: 'lead-2', agentflowid: 'flow-1', name: 'Jane Doe', email: 'jane@example.com' }
                ]
                ...
                expect(mockRepository.find).toHaveBeenCalledWith({ where: { agentflowid: 'flow-1' } })
            })
        ...
        describe('createLead', () => {
            it('should create a new lead with provided chatId', async () => {
                const leadData = { agentflowid: 'flow-1', chatId: 'custom-chat-id', name: 'John Doe', email: 'john@example.com' }
            ...
            it('should generate chatId when not provided', async () => {
                ...
                const result = await leadsService.createLead({ agentflowid: 'flow-1', name: 'Jane Doe' })
            ...
            it('should throw InternalChronosError on save error', async () => {
                ...
                await expect(leadsService.createLead({ agentflowid: 'flow-1' })).rejects.toThrow('Error: leadsService.createLead')
            })
        })
    })
}
