...
class ExtendedLunaryHandler extends LunaryHandler {
    ...
    constructor({ chronosOptions, ...options }: any) {
        super(options)
        this.appDataSource = chronosOptions.appDataSource
        this.databaseEntities = chronosOptions.databaseEntities
        this.chatId = chronosOptions.chatId
        this.apiMessageId = chronosOptions.apiMessageId
    }
...
export class AnalyticHandler {
    ...
    async onChainStart(name: string, input: string, parentIds?: ICommonObject) {
        ...
        if (Object.prototype.hasOwnProperty.call(this.handlers, 'arize')) {
            ...
            if (!parentIds || !Object.keys(parentIds).length) {
                rootSpan = tracer ? tracer.startSpan('Chronos') : undefined
                if (rootSpan) {
