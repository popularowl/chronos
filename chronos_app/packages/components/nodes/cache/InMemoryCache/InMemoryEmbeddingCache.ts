...
import { CacheBackedEmbeddings } from '../../../src/compat/embeddings'
...
class InMemoryEmbeddingCache implements INode {
    ...
    async init(nodeData: INodeData, _: string, options: ICommonObject): Promise<any> {
        ...
        const memoryMap = (await options.cachePool.getEmbeddingCache(options.agentflowid)) ?? {}
        const inMemCache = new InMemoryEmbeddingCacheExtended(memoryMap)

        inMemCache.mget = async (keys: string[]) => {
            const memory = (await options.cachePool.getEmbeddingCache(options.agentflowid)) ?? inMemCache.store
        ...
        inMemCache.mset = async (keyValuePairs: [string, any][]): Promise<void> => {
            ...
            await options.cachePool.addEmbeddingCache(options.agentflowid, inMemCache.store)
        }
